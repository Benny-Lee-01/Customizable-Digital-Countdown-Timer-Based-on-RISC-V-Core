...
* **系統啟動與初始化**: 燒錄完成後，頂層模組會自動發送一次開機自動重置訊號，系統直接跳過手動重置流程並即時讀取下方的 slide switch 開關。
* **自訂初始時間**: 透過撥動板上 16 個switch `SW[15:0]` 輸入目標秒數，此時上方的七段顯示器會即時將其轉換為十進位制並顯示（最高支援 9999 秒）。
...
* **輪詢迴圈功耗代價**: 目前 1Hz 的硬體計時狀態抓取與按鈕偵測完全依賴 CPU 執行 Polling迴圈，導致處理器長時間處於滿載運作，增加了整體電路的運作功耗。未來改善方向為整合硬體中斷控制器，使處理器在閒置時進入低功耗休眠。
